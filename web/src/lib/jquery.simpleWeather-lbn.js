/*
 * Customized version of simpleWeather
 * by ixdl
 *
 * ---------------------
 * simpleWeather
				...
				function(data) {
					if(data !== null && data.query.results !== null && data.query.results.channel.description !== 'Yahoo! Weather Error') {
						$.each(data.query.results, function(i, result) {
							...
							for (i=1; i<=3; ++i) {
								items.push({
									high: result.item.forecast[i].high,
									low: result.item.forecast[i].low,
									forecast: result.item.forecast[i].text,
									code: result.item.forecast[i].code,
									date: result.item.forecast[i].date,
									day: result.item.forecast[i].day,
									image: "http://l.yimg.com/a/i/us/nws/weather/gr/"+result.item.forecast[i].code+"d.png"
								});
							}
